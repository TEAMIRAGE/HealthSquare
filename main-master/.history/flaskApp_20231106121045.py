...
def fracture_process(img_name):
    try:
        ...
        service_account_file = 'linear-yen-400506-78e0707a1328.json'
        ...
        if fracture_result is None:
            ...
            return jsonify({"message": "The provided image is not Correct."}), 201
        elif fracture_result:
            print("Fracture is present.")
            return jsonify({"message": "Fracture is present."}), 201
        else:
            print("No fracture is detected.")
            return jsonify({"message": "No fracture is detected."}), 201
