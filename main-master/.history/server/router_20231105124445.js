    ...
    try {
      ...
      if (selectedOption === "Fracture") {
        ...
        stream.on("finish", async () => {
          ...
          const response = await axios.get(
            `${ip1}${port1}/fracture-Processing-result/${gcpBucketName}`
          );
          console.log(response.data);
          const updatedData = await fracture_Database.findByIdAndUpdate(
            data._id,
            {
          ...
          const finalData = await axios.get(
            `${ip}${port}/api/fractureImage?id=${data._id}`
          );
          ...
          if (finalData.data.output.includes("No fracture is detected")) {
            ...
            const newupdatedData = await fracture_Database.findByIdAndUpdate(
              data._id,
              {
                output: resultString,
              },
            ...
            const userName = await userProfile_Database.findById(data.UserId);
            const deleteData = await fracture_Database.findByIdAndDelete(
              data._id
            );
            res.render("resultOutput.ejs", {
              name: userName.name,
      ...
      // %%%%%%%%%%%%%%%%%%%%% TUMOR START %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        stream.on("finish", async () => {
          ...
          const response = await axios.get(
            `${ip}${port}/tumor-Processing-result/${data._id}`
          );
          console.log(response.data);

          const updatedData = await tumor_Database.findByIdAndUpdate(
            data._id,
            {
              output: response.data,
            },
          ...
          const finalData = await axios.get(
            `${ip}${port}/api/tumorImage?id=${data._id}`
          );
          ...
          if (finalData.data.output.includes("no_tumor")) {
            ...
            console.log("String value: ", resultString);
            console.log("Result Image value: ", resultImage);
            const newupdatedData = await tumor_Database.findByIdAndUpdate(
              data._id,
              {
                output: resultString,
              },
...
route.post("/api/manual/UploadData", async (req, res) => {
  try {
    ...
    const Data = await axios.get(
      `${ip}${port}/bloodReport-manual-Processing-result`,
      {
        params: {
          bloodReport_hemoglobin,
          bloodReport_rbc,
...
route.get(
  "/bloodReport-manual-Processing-result",
  pythonProcess.bloodReport_Process_manual
);
