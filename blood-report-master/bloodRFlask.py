...
def blood_report():
    try:
        ...
        input_text = f"""input:My Blood Report values of "White Blood Cell" is {bloodReport_wbc} /cu.mm,
        "Red Blood Cell" is {bloodReport_rbc} g%,"Hemoglobin" is {bloodReport_hemoglobin} g%,
        "Hematocrit"is {bloodReport_hematocrit} %,"MCV"is {bloodReport_mcv} fl,
        "MCH" is {bloodReport_mch} pg,"MCHC" is {bloodReport_mchc} %,
        "Platelet" is {bloodReport_platelet} /cu.mm,"Neutrophils" is {bloodReport_neutrophils} %,
        "Lymphocytes" is {bloodReport_lymphocytes} %,"Monocytes" is {bloodReport_monocytes} %,
        "Eosinophils" is {bloodReport_eosinophils} % give me a detailed diagnosisof the report considering there units."""

        # Generate the response
        response = model.predict(input_text, **parameters)
        # Extract the text after "Overall Interpretation:"
        overall_interpretation_start = response.text.find("**Overall Interpretation:**")

        # Get the text from the start of "Overall Interpretation:" to the end of the string
        overall_interpretation_text = response.text[overall_interpretation_start:]
        # Output the extracted text
        print(overall_interpretation_text)
        ...
        blood_test_results = [
            ...
            ("Hematocrit", f"{bloodReport_hematocrit}", "35 -50 %"),
        ...
        # Now, create the diagnosis string with preserved line breaks
        diagnosis_text = overall_interpretation_text
        diagnosis_paragraphs = [Paragraph(f"Diagnosis: <b>{para}</b>", normal_style) for para in diagnosis_text.split('\n')]
        elements.extend(diagnosis_paragraphs)
        elements.append(Spacer(1, 12))

        # Add "generated by medilabs.com" at the bottom corner
        generated_by = Paragraph("Generated by HealthSquare", ParagraphStyle(
            'Normal',
            fontSize=10,
        ...
        client = storage.Client().from_service_account_json("pure-silicon-390116-5d3c01f54cc0.json")
        # Get the bucket
